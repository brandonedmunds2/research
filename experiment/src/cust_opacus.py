...
class AmountDPOptimizer(DPOptimizer):
    ...
    def add_noise(self):
        for i,p in enumerate(self.params):
            ...
            if(self.amount != 0):
                F.dropout(noise, p=self.amount, training=True, inplace=True)
                noise.mul_(self.amount)
            p.grad = (p.summed_grad + noise).view_as(p)

            _mark_as_processed(p.summed_grad)

class AmountPrivacyEngine(PrivacyEngine):
    def __init__(self, amount, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.amount=amount
